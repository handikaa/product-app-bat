# product_bat

Flutter version using : 3.32.5
Dart version using : 3.8.1

## Getting Started
## App Description
Aplikasi sederhana untuk simulasi toko online

## Feature
Fitur : 
1. List Produk
2. Cari produk
3. Lihat halaman detail produk
4. Keranjang (tambah, hapus, edit)

## Package used 
  cupertino_icons: ^1.0.8
  bloc: ^9.1.0
  flutter_bloc: ^9.1.1
  dio: ^5.9.0
  dartz: ^0.10.1
  get_it: ^9.0.5
  shimmer: ^3.0.0
  go_router: ^17.0.0
  equatable: ^2.0.7
  intl: ^0.20.2
  lottie: ^3.3.1
  flutter_rating_bar: ^4.0.1
  hive: ^2.2.3
  hive_flutter: ^1.1.0
  flutter_slidable: ^4.0.3

## How to run the project
1. clone github https://github.com/handikaa/product-app-bat.git di IDE
2. set folder project di folder yang ditentukan
3. buka folder project product-app-bat di IDE
4. jalankan "flutter run" di terminal, atau run debugging langsung jika menggunakan VS Code



# UI preview
<img width="1080" height="2340" alt="Screenshot_1763117170" src="https://github.com/user-attachments/assets/30784f9e-e2c4-4f3d-a080-11570e5dba59" />
<img width="1080" height="2340" alt="Screenshot_1763117175" src="https://github.com/user-attachments/assets/7259dc82-8ef5-4c39-97dd-bca07f5592cf" />
<img width="1080" height="2340" alt="Screenshot_1763117178" src="https://github.com/user-attachments/assets/1c921e4a-0a5d-4434-9fe5-4085780221c2" />




